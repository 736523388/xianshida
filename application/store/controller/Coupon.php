...
//            $data['use_level'] = explode(',', isset($data['use_level']) ? $data['use_level'] : '');
            ...
            if(!isset($data['time_type'])) $data['time_type'] = 2;
            if(empty($data['coupon_day'])) $this->error("请输入有效期天数");
            if($data['time_type'] == 1){
            ...
            $coupon_auth_type_arr = [
                2 => ['key' => 'coupon_auth_cate', 'title' => '商品分类', 'type' => 'change'],
                3 => ['key' => 'coupon_auth_brand', 'title' => '商品品牌', 'type' => 'change'],
                4 => ['key' => 'coupon_auth_goods', 'title' => '商品', 'type' => 'change'],
            ];
            if(in_array(intval($data['coupon_auth_type']), [2,3,4])){
                $auth_key = $coupon_auth_type_arr[intval($data['coupon_auth_type'])]['key'];
                $auth_title = $coupon_auth_type_arr[intval($data['coupon_auth_type'])]['title'];
                if(!isset($data[$auth_key]) || empty($data[$auth_key])){
                    $this->error("请选择{$auth_title}");
                }
            }
            //dump($data);exit();
        }
